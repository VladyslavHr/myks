...
<div class="container py-3">
    <div class="row py-3">
        <div class="col-md-2">
            <a class="btn " href="{{ route('admin.videoLecture.index') }}">
    ...
    <form class="mt-3" action="{{ route('admin.videoLecture.update', $video) }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="row">
            <div class="mb-3 col-md-4">
                <label for="" class="form-label">Назва відео</label>
                <input name="title" value="{{ $video->title }}" type="text" class="form-control" id="">
                <span class="form-text">Будь ласка напишіть назву відео.</span>
            </div>
            <div class="mb-3 col-md-4">
                <label for="" class="form-label">Посилання на відео</label>
                <input name="video" value="{{ $video->video }}" type="text" class="form-control" id="">
            ...
            <div class="mb-3 col-md-4">
                ...
                <label for="">{{ $video->category->title }}</label>
                <select class="form-select" name="product_category_id" >
                    @foreach ($categories as $category)
                        <option {{ $category->id == $video->category->id ? 'selected' : '' }} value="{{$category->id }}">{{ $category->title }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-3 col-md-6">
                ...
                <input name="seo_title" value="{{ $video->seo_title }}" type="text" class="form-control" id="" placeholder="">
                <span class="form-text">Будь ласка напишіть SEO заголовок.</span>
            </div>
            <div class="mb-3 col-md-6">
                <label for="" class="form-label">SEO ключові слова</label>
                <input name="seo_keywords" value="{{ $video->seo_keywords }}" type="text" class="form-control" id="" placeholder="">
                <span class="form-text">Будь ласка напишіть SEO ключові слова.</span>
            </div>
            <div class="mb-3 col-md-6">
                <label for="" class="form-label">SEO опис</label>
                <input name="seo_description" value="{{ $video->seo_description }}" type="text" class="form-control" id="" placeholder="">
